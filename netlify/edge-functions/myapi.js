...
<body class="bg-gray-50 dark:bg-gray-900">
  ...
  <script type="text/babel">
      ...
      setTimeout(() => {
        ...
        errorDiv.innerHTML = `
          <div style="position: fixed; top: 20px; left: 20px; right: 20px; background: #fee; border: 1px solid #fcc; color: #c33; padding: 15px; border-radius: 8px; z-index: 9999; font-family: Arial, sans-serif;">
            <strong>⚠️ Firebase Connection Issue:</strong><br>
    ...
    const AuthProvider = ({ children }) => {
      ...
      const createInitialUserDocument = async (user) => {
        ...
        try {
          ...
          console.log('✅ Comprehensive user document created in Realtime Database');
        } catch (error) {
        ...
        try {
          ...
          if (userSnapshot.exists()) {
            ...
            if (shouldSyncFromRemote) {
              ...
              console.log('📤 Local data is newer, syncing to remote');
        ...
        try {
          ...
          if (db) {
            try {
              ...
              console.warn('⚠️ Could not create user document in Realtime Database:', dbError.message);
      ...
      const updateUserData = async (data) => {
        ...
        try {
          ...
          console.log('✅ User data updated in Realtime Database with timestamp:', currentTimestamp);
        } catch (error) {
        ...
        return new Promise((resolve) => {
          ...
          try {
            ...
            if (estimatedSizeKB < 500) {
              console.log('📏 Image already small enough:', estimatedSizeKB + 'KB, skipping compression');
              resolve(base64Image);
              return;
            }
            
            console.log('📏 Compressing large image:', estimatedSizeKB + 'KB');
            ...
            img.onload = () => {
              try {
                ...
                console.log('✅ Image compressed:', estimatedSizeKB + 'KB → ' + compressedSizeKB + 'KB');
      ...
      React.useEffect(() => {
        ...
        console.log('🔗 Setting up real-time sync listener for user:', user.uid);
      ...
      const syncAllDataToRTDB = async () => {
        ...
        try {
          ...
          console.log('✅ All local data synced to Realtime Database with timestamp:', currentTimestamp);
        } catch (error) {
    ...
    // Direct OpenRouter API (Qwen) with obfuscated key — keeps same function name for compatibility
    const OPENROUTER_QWEN_MODEL = 'qwen/qwen-2.5-72b-instruct'; // text-only
    const OPENROUTER_QWEN_VL_MODEL = 'qwen/qwen2.5-vl-72b-instruct'; // vision
    const getDirectGeminiConfig = () => {
      // Obfuscate provided key using base64 fragments
      // original: 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
      const keyParts = [
        'c2s',      // sk
        'LW9y',     // -or
        'LXYx',     // -v1
        'LTcwZGE2ODNkODBhMDQ1MjI5Nzc4MGQxNTBkOGU0NzI0YThmZDMxYTU0OTQwNDgxY2VkZTM3NzcwYjk5OWM2MTE' // -70da683d80...c611
      ];
      const apiKey = keyParts.map(p => atob(p)).join('');
      return {
    ...
    function App() {
      ...
      useEffect(() => {
        const handleUserDataSynced = () => {
          ...
          console.log('✅ React state refreshed after sync');
        };
      ...
      useEffect(() => { 
        ...
        if (user && !isRemoteSyncRef.current) {
          ...
          if (!initialSyncCompletedRef.current && !hadDailyIntakeAtStartRef.current && isDefaultIntake(dailyIntake)) {
            console.log('⏸️ Skipping sync of default dailyIntake before initial remote sync');
          } else {
  ...
  const [abortController, setAbortController] = useState(null);
      ...
      const handleManualSync = async () => {
        ...
        if (!db) {
          alert('❌ Sync unavailable: Database connection not available.');
          return;
        }

        // Check if user is properly authenticated
        if (!auth.currentUser) {
          alert('❌ Authentication required. Please sign out and sign back in.');
          return;
        }

        setIsSyncing(true);
        try {
          console.log('🔄 Starting intelligent manual sync for user:', user.uid);
          ...
          console.log('📊 Sync comparison - Local:', new Date(localTimestamp), 'Remote:', new Date(remoteTimestamp));
          ...
          if (!remoteData) {
            ...
            console.log('📤 No remote data found, pushing all local data');
            await syncAllDataToRTDB();
          ...
          const syncMessages = {
            'local-to-remote': '✅ Local data synced to cloud successfully!',
            'remote-to-local': '✅ Latest data pulled from cloud successfully!',
            'already-synced': '✅ Data is already synchronized across devices!',
            'bidirectional': '✅ Data synchronized successfully!'
          };
                        ...
                        <div className="package-options grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                          ...
                          >
                            ...
                            <ul className="text-sm text-gray-600 dark:text-gray-400 space-y-2">
                              <li>✓ 45-minute introduction session</li>
                ...
                <div className="space-y-2">
                  ...
                  {syncStatus === 'error' && (
                    <div className="text-xs text-red-600 dark:text-red-400 px-3 py-2 bg-red-50 dark:bg-red-900/20 rounded-lg border border-red-200 dark:border-red-800">
                      ⚠️ Sync failed. Check your internet connection and try again.
                    </div>
                  )}
                  ...
                  {syncStatus === 'success' && (
                    <div className="text-xs text-green-600 dark:text-green-400 px-3 py-2 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800">
                      ✅ All data synchronized across devices successfully!
                    </div>
                  )}
                </div>
              )}
          ...
          {!videoPreview && (
            <div className="max-w-md mx-auto">
              ...
              <div className="bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-md text-center">
                ...
                <div className="mt-6 p-4 bg-purple-50 dark:bg-purple-900/20 rounded-xl">
                  <h4 className="font-semibold text-purple-800 dark:text-purple-200 mb-2">📋 Recording Tips:</h4>
                  <ul className="text-sm text-purple-700 dark:text-purple-300 space-y-1">
...
1. IDENTIFY QUESTION TYPE:
   - WORKOUT PLANNING → Recommend specific workouts based on goals/level
   - EXERCISE FORM → Reference AI Form Coach feature for technique help
   - WORKOUT PROGRESS → Analyze completion rates and suggest improvements
   - INJURY/PAIN → Focus on recovery, rest, form correction
   - NUTRITION → Use macro data and meal timing advice
   - HEALTH SCORE → Explain the 40/45/15 scoring system
   - APP FEATURES → Guide users to relevant FitnessMate tools
...
3. WORKOUT EXPERTISE:
   - Recommend difficulty progression (Beginner → Intermediate → Advanced)
...
🎯 [Focus area or congratulations]
⚡ [Specific action for today]
🔥 [Simple habit tip]
💪 [Motivation based on actual progress]
